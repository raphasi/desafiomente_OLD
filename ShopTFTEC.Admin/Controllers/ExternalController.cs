...
namespace IdentityServerHost.Quickstart.UI
{
    ...
    public class ExternalController : Controller
    {
        ...
        private async Task<ApplicationUser> AutoProvisionUserAsync(string provider, string providerUserId, IEnumerable<Claim> claims)
        {
            ...
            var roles = claims.FirstOrDefault(x => x.Type == JwtClaimTypes.Role)?.Value ??
                claims.FirstOrDefault(x => x.Type == ClaimTypes.Role)?.Value;
            if (roles != null)
            {
                filtered.Add(new Claim(JwtClaimTypes.Role, roles));
            }
            ...
            if (filtered.Any())
            {
                ...
                identityResult = await _userManager.AddToRoleAsync(user, roles);
                if (!identityResult.Succeeded) throw new Exception(identityResult.Errors.First().Description);
